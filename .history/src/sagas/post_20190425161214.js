...
function* FetchPost(action){
    try{
        console.log("render")
        const home = yield call(axios.get,"https://cnodejs.org/api/v1/topics",{params:action.params})
        yield put({type:FETCH_POST_SUCCEEDED,post:home.data.data}) //相当于dispatch一个action,并把值传过去
    }catch(e){
...
export function* watchFetchHome(){
    yield takeEvery(HOMEDATA,FetchPost)
}
